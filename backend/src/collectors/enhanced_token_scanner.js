...
class EnhancedTokenScanner {
    ...
    async startScanning() {
        ...
        this.startPumpFunMonitoring();
    }

    async scanAll() {
        const [dexScreenerTokens, birdeyeTokens, raydiumPools, pumpFunTokens] = await Promise.all([
            this.fetchDexScreenerTokens(),
            this.fetchBirdeyeTokens(),
            this.fetchRecentRaydiumPools(),
            this.fetchRecentPumpFunTokens()
        ]);

        const allTokens = [...dexScreenerTokens, ...birdeyeTokens, ...raydiumPools, ...pumpFunTokens];
        ...
        logger.info(`Found ${newTokens.length} new tokens to analyze (including ${pumpFunTokens.length} from Pump.fun)`);
    ...
    async startPumpFunMonitoring() {
        try {
            ...
            this.pumpFunScanner.on('token', async (tokenInfo) => {
                logger.info(`🎯 Pump.fun token detected: ${tokenInfo.symbol}`);
                
                // Avoid duplicates
                if (this.processedTokens.has(tokenInfo.address)) {
                    return;
                }
                
                // Store in pump.fun tokens map
                this.pumpFunTokens.set(tokenInfo.address, tokenInfo);
                
                // Process immediately with high priority
                await this.analyzeAndStoreToken({
                    ...tokenInfo,
                    priority: 'high',
                    source: 'pumpfun'
                });
                
                this.processedTokens.add(tokenInfo.address);
                
                // Clean up old pump.fun tokens
                if (this.pumpFunTokens.size > 50) {
                    const oldest = Array.from(this.pumpFunTokens.keys()).slice(0, 25);
                    oldest.forEach(key => this.pumpFunTokens.delete(key));
                }
            });
            
            // Start the scanner
            await this.pumpFunScanner.start(this.connection);
            logger.info('✅ Pump.fun monitoring initialized');
    ...
    async fetchRecentPumpFunTokens() {
        try {
            // Get recent tokens from the pump.fun scanner
            const recentTokens = await this.pumpFunScanner.getRecentTokens(20);
            
            // Enrich with current data if possible
            const enrichedTokens = [];
            
            for (const token of recentTokens) {
                try {
                    // Try to get current price/liquidity data
                    const enriched = await this.enrichPumpFunToken(token);
                    enrichedTokens.push(enriched);
                } catch (error) {
                    // Use original token data if enrichment fails
            ...
            logger.warn('Error fetching pump.fun tokens:', error);
            return [];
        }
    }

    async enrichPumpFunToken(token) {
        try {
            ...
            if (response.data?.pairs && response.data.pairs.length > 0) {
                ...
                return {
                    ...
                    liquidity: parseFloat(pair.liquidity?.usd || token.liquidity),
            ...
            // Enrichment failed, use original data
        }
    ...
    deduplicateTokens(tokens) {
        ...
        for (const token of tokens) {
            ...
            // Prioritize pump.fun tokens or Raydium direct tokens
            if (!existing || 
                token.source === 'pumpfun' || 
                token.source === 'raydium-direct' ||
                (existing.source === 'dexscreener' && token.source !== 'dexscreener')) {
    ...
    async analyzeAndStoreToken(token) {
        try {
            // Give bonus points for pump.fun discoveries
            const sourceBonus = token.source === 'pumpfun' ? 25 : 
                              token.source === 'raydium-direct' ? 20 : 0;
            
            // Extra bonus for very new pump.fun tokens
            const ageMinutes = (Date.now() - token.createdAt) / (1000 * 60);
            const freshnessBonus = token.source === 'pumpfun' && ageMinutes < 5 ? 15 : 0;
            ...
            const analysis = {
                ...
                ageScore: this.scoreAge(token.createdAt) + sourceBonus + freshnessBonus,
            ...
            // Lower threshold for pump.fun discoveries
            const scoreThreshold = token.source === 'pumpfun' ? 15 : 
                                 token.source === 'raydium-direct' ? 20 : 30;
            
            if (overallScore > scoreThreshold && riskScore < 90) {
                await this.db.addToken({
                    ...
                    liquidity: token.liquidity,
                    holders: 0,
                    socialScore: token.isNew ? 10 : 0,
                    riskScore: riskScore,
                    source: token.source || 'unknown',
                    metadata: JSON.stringify({
                        source: token.source,
                        deploymentType: token.deploymentType,
                        isPumpFun: token.isPumpFun || false
                    }),
                    isPumpFun: token.source === 'pumpfun',
                    deploymentType: token.deploymentType
                });
    ...
    stopScanning() {
        ...
        // Stop pump.fun scanner
        if (this.pumpFunScanner) {
